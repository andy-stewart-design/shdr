...
void main() {
    ...
    float pixelHue = abs(cnoise(vec3(pixel, u_time * u_speed)));
    ...
    float rad = (1. - pixelBrightness) * 0.75;
    ...
    vec3 gradientColor = palette(uv.x * sin(1.0) + uv.y);
    vec3 gradientColorGrained = palette(uv.x * sin(1.0) + uv.y + grain);

    gradientColor = mix(gradientColorGrained, gradientColor, 0.925);
    gradientColor = mix(black, gradientColor, 0.9);

    vec3 finalColor = vec3(0., 0., 1.) * d;
